...
* Forecast settings
  * The time-varying Pstar for your forecast.ss file can be calculated using `PEPtools::get_buffer()` (no website, see (PEPtools github repo)[https://github.com/pfmc-assessments/PEPtools/])
    * default `sigma` input to that function is 0.5 for Category 1 stocks and 1.0 for Category 2 but if uncertainty in the model (one of `model$Pstar_sigma` or `model$OFL_sigma` as output from `r4ss::SS_output()`--check with SSC folks on which) is larger, use that instead.
  ...
  * Fixed catches for first 2 years of forecast 
    * typically provided by GMT rep to STAR
    * OFL/ACL values are in GMT016 table in https://reports.psmfc.org/pacfin/f?p=501:1000:4357688221402::::: (remember to filter for your species)
* Creating the forecast table of OFL/ABC/ACL
  * You can use `r4ss::SSexecutivesummar()` to help with this task 
    * You may want to specify the OFLs (and ACLs) for the next two years: `r4ss::SSexecutivesummary(model, forecast_ofl = c(3763, 3563))
