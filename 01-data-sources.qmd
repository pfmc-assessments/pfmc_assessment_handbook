...
Note: the names for the first 4 of these surveys in the [{nwfscSurvey}](https://github.com/pfmc-assessments/nwfscSurvey) package are "NWFSC.Combo", "Triennial", "AFSC.Slope", "NWFSC.Slope". The H&L and acoustic surveys are not currently available through that package.
...
The GEMM report is produced by the WGCOP team with Dr. Kayleigh Somers ([kayleigh.somers\@noaa.gov](mailto:kayleigh.somers@noaa.gov)) as the lead author and is published in the September Council meeting Briefing Book titled the "Groundfish Mortality Report". The GEMM can be found within the associated Excel spreadsheet on the Table 3 tab. Additionally, the GEMM can be accessed via two other methods: 1) the GEMM report is archived on the network under \\Assessments\\GEMM Report or 2) can be accessed using the [{nwfscSurvey}](https://github.com/pfmc-assessments/nwfscSurvey) using the `pull_gemm()` function.
